...
def isvalid(field, value):
    if field == "byr":
        return validinrange(value, 1920, 2002)
    elif field == "iyr":
        return validinrange(value, 2010, 2020)
    elif field == "eyr":
        return validinrange(value, 2020, 2030)
    elif field == "hgt":
        if re.match("[0-9]+(cm|in)", value):
            num = value[0: -2]
            unit = value[-2:]
            if unit == "cm" and validinrange(num, 150, 193):
                return True
            if unit == "in" and validinrange(num, 59, 76):
                return True
    elif field == "hcl":
...
def validate(passport):
    ...
    for line in passport.split("\n"):
        ...
        for entry in entries:
            ...
            if field == "cid":
                continue
            passport_fields[field] = isvalid(field, value)
