...
function Is-Basic-Passphrase-Valid {
   param([string] $passphrase)
   $unique_words = @{}
   $words = $passphrase.Split(" ")
   $words | foreach-object { $unique_words.Set_Item($_, $_) }
   
   $has_multiple_words = $passphrase.Split(" ").Length -gt 1
   $no_duplicates = $unique_words.Count -eq $words.Count
   
   $is_valid = $has_multiple_words -and $no_duplicates
   return $is_valid
}

function Is-Advanced-Passphrase-Valid {
   param([string] $passphrase)
   if (-not (Is-Basic-Passphrase-Valid $passphrase)) {
      return $FALSE
   }
   ...
   foreach ($word in $words) {
      $chars = [string]::Join("", ([char[]]$word | sort-object))
      $unique_anagrams.Set_Item($chars, $chars)
   }
   
   $has_no_anagrams = $words.Count -eq $unique_anagrams.Count
   return $has_no_anagrams
}

Write-Host (Is-Basic-Passphrase-Valid "aa bb cc dd ee")
Write-Host (Is-Basic-Passphrase-Valid "aa bb cc dd aa")
Write-Host (Is-Basic-Passphrase-Valid "aa bb cc dd aaa")
$input.Split([Environment]::NewLine) | foreach-object { Is-Basic-Passphrase-Valid $_ } | where-object { $_ -eq $TRUE } | measure-object | select -Property Count

Write-Host (Is-Advanced-Passphrase-Valid "abcde fghij")
Write-Host (Is-Advanced-Passphrase-Valid "abcde xyz ecdab")
Write-Host (Is-Advanced-Passphrase-Valid "a ab abc abd abf abj")
Write-Host (Is-Advanced-Passphrase-Valid "iiii oiii ooii oooi oooo")
Write-Host (Is-Advanced-Passphrase-Valid "oiii ioii iioi iiio")
$input.Split([Environment]::NewLine) | foreach-object { Is-Advanced-Passphrase-Valid $_ } | where-object { $_ -eq $TRUE } | measure-object | select -Property Count
