...
class Program {
    ...
    public function __toString() {
        return $this->callerName . " " . $this->weight . " -> " . implode(",", $this->calleeNames);
    }
    
    public static function fromString($string) {
        $program = new Program();
        
        $callStack = explode(" -> ", $string);
        $caller = $callStack[0];
        $call = explode(" ", $caller);
        $program->callerName = trim($call[0]);
        $program->weight = trim($call[1]);

        if(count($callStack) > 1) {
            $callees = $callStack[1];
            $program->calleeNames = explode(", ", trim($callees));
        }
        else {
            $program->calleeNames = [];
        }
...
function createPrograms($input) {
    ...
    $callees = [];
    $weights = [];
    foreach($programList as $programCalls) {
       $program = Program::fromString($programCalls);
       $programs[$program->callerName] = $program;
       $program->weight = str_replace("(", "", $program->weight);
       $program->weight = str_replace(")", "", $program->weight);
       $program->weight = intval($program->weight);
       $weights[$program->callerName] = $program->weight;
       if (count($program->calleeNames) > 0) {
            foreach($program->calleeNames as $callee) {
                $callees[$callee] = $program->callerName;
            }
       }
    }
    return array(
...
function printRoot($input) {
    $programs = createPrograms($input); 
    $result = findRoot($programs["programs"], $programs["callees"]);
    calculateWeights($programs["programs"], $programs["callees"], $programs["weights"]);
    //print_r($GLOBALS['totalWeights']);
    print $result . "\n";
}

function calculateWeight($program, $programs, $callees, $weights) {
    if(isset($GLOBALS['totalWeights'][$program->callerName])) {
        return $GLOBALS['totalWeights'][$program->callerName];
    }
    if(count($program->calleeNames) == 0) { 
        $GLOBALS['totalWeights'][$program->callerName] = $program->weight;
        return $GLOBALS['totalWeights'][$program->callerName];
    }
    ...
    $currentCallees = [];
    foreach($program->calleeNames as $callee) {
        $currentWeight = calculateWeight($programs[$callee], $programs, $callees, $weights);
        array_push($currentWeights, $currentWeight);
        array_push($currentCallees, $programs[$callee]);
        if(!isset($currentWeightCounts[$currentWeight])) {
            $currentWeightCounts[$currentWeight] = 0;
        }
        $currentWeightCounts[$currentWeight]++;
        $weight += calculateWeight($programs[$callee], $programs, $callees, $weights);
    }
    $uniqueWeights = array_values(array_unique($currentWeights));
    if (count($uniqueWeights) != 1) {
        $rightIndex = 0;
        $wrongIndex = 1;
        if ($currentWeightCounts[$uniqueWeights[0]] == 1) {
            $wrongIndex = 0;
            $rightIndex = 1;
        }
        $rightWeight = $uniqueWeights[$rightIndex];
        $wrongWeight = $uniqueWeights[$wrongIndex];

        $wrongCalleeIndex = array_search($wrongWeight, $currentWeights);
        $wrongProgram = $currentCallees[$wrongCalleeIndex];
        $wrongProgramWeight = $wrongProgram->weight;

        $diff = $wrongProgramWeight - ($wrongWeight - $rightWeight);
        print $diff . " \n";
    }

    $GLOBALS['totalWeights'][$program->callerName] = $weight;
    return $weight;
}

function calculateWeights($programs, $callees, $weights) {
    $GLOBALS['totalWeights'] = [];
    foreach($programs as $program) {
        calculateWeight($program, $programs, $callees, $weights);
        if (count($GLOBALS['totalWeights']) == count($programs)) {
            break;
        }
    }
}

function findRoot($programs, $callees) {
    $calls = [];
    foreach($programs as $program) {
        $calls[$program->callerName] = 0;
    }
    foreach($callees as $callee => $caller) {
        if(isset($calls[$callee])) {
            $calls[$callee]++;
        }
    }
    foreach($calls as $caller => $count) {
        if($count == 0) {
            return $caller;
        }
    }
