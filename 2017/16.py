...
def printa(a, swap_count):
    b = [0 for i in range(0, len(a))]
    for i in range(0, len(a)):
        b[(i+swap_count) % len(a)] = chr(a[i]+97)
    print(''.join(b))
    print()

byPos = array('B', [0,1,2,3,4])
byChar = array('B', [0,1,2,3,4])
swap_count = 0
printa(byPos, swap_count)

swap_count = 1
printa(byPos, swap_count)

A, B = byPos[3-swap_count], byPos[4-swap_count]
byPos[3-swap_count], byPos[4-swap_count] = B, A
printa(byPos, swap_count)
