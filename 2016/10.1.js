var input = `bot 171 gives low to bot 4 and high to bot 84
...
for(var i = 0; i < lines.length; i++)
{
 ...
 if (parts[0] === 'value' && j === 0)
 {
  var value = parseInt(parts[1]);
  var bot = parseInt(parts[5]);
  bots[bot] ? bots[bot].push(value) : bots[bot] = [value];
 }
 else if (parts[0] === 'bot' && j === 1)
 {
  var bot = parseInt(parts[1]);
  var low_target = parts[5];
  var low_index = parseInt(parts[6]);
  var low_value = Math.min(...bots[bot]);
  if (low_target === 'bot'){
   bots[low_index] ? bots[low_index].push(low_value) : bots[low_index] = [low_value];
  }
  else if (low_target === 'output') {
   outputs[low_index] ? outputs[low_index].push(low_value) : outputs[low_index] = [low_value];
  }
  bots[bot].splice(bots[bot].indexOf(low_value), 1);
  
  var high_target = parts[10];
  var high_index = parseInt(parts[11]);
  var high_value = Math.min(...bots[bot]);
  if (high_target === 'bot'){
   bots[high_index] ? bots[high_index].push(high_value) : bots[high_index] = [high_value];
  }
  else if (high_target === 'output') {
   outputs[high_index] ? outputs[high_index].push(high_value) : outputs[high_index] = [high_value];
  }
  bots[bot].splice(bots[bot].indexOf(high_value), 1);
  
  if (low_value === low_compare && high_value === high_compare)
  {
   console.log('bot ' + bot + ' is responsible for comparing ' + low_compare + ' and ' + high_compare);
  }
 }
}
}
