...
for(var i = 0; i < lines.length; i++) {
  ...
  var enc_name = match[1];
  var sector = parseInt(match[2]);
  var checksum = match[3];

  var name = enc_name.replace(/-/g,'');
  var count = {};
  ...
  var chars = list.map(function (item) {
    return item.char;
  }).slice(0,5).join('');
  //console.log(list);
  sum += chars === checksum ? sector : 0;
  //console.log(checksum+ ' '+ chars + ' ' +line);
  
  //part 2
  var aCode = 'a'.charCodeAt(0);
  var dec_name = enc_name.replace(/-/g,' ').split('').map(function (char) {
   if (char===' ') { return char; }
   var code = char.charCodeAt(0) - aCode;
   var newCode = (code + sector) % 26;
   return String.fromCharCode(newCode + aCode);
  }).join('');
  //console.log(dec_name);
  if (dec_name.indexOf('north') > -1) {
   console.log(dec_name + ' - ' + sector);
  }
}
