var inputs = `rhamaeovmbheijj[hkwbkqzlcscwjkyjulk]ajsxfuemamuqcjccbc
...
for(i=0; i<lines.length;i++) {
 var supernets = [];
 var supernet = /\[(\w+)\]/g;
 while (matches = supernet.exec(lines[i])) {
    supernets.push(matches[1]);
 }
 var hypernets = [];
 var hypernet = /(^|\])(\w+)(\[|$)/g;
 while (matches = hypernet.exec(lines[i])) {
    hypernets.push(matches[2]);
 }
 var ABAs = [];
 supernets.forEach(function (supernet) {
  for(var j=0; j < supernet.length-2; j++)
  {
   var ABA = supernet[j] + supernet[j+1] + supernet[j+2];
 ...
 var ssl = false;
 hypernets.forEach(function (hypernet) {
  for(var j=0; j < hypernet.length-2; j++)
  {
   var BAB = hypernet[j] + hypernet[j+1] + hypernet[j+2];
   var ABA = BAB[1] + BAB[0] + BAB[1];
   if (BAB[0] === BAB[2] && BAB[0] !== BAB[1] && ABAs.indexOf(ABA) > -1)
   {
    ssl = true;
   }
  }
